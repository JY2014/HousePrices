...
for alpha1 in alpha1_list:
    ...
    y_pred_l1 = cross_val_predict(elastic_l1, x_train_std, y_train, cv = skf)
    ...
    for alpha2 in alpha2_list:
        ...
        y_pred_l2 = cross_val_predict(elastic_l2, x_train_std, y_train, cv = skf)
...
# tune lasso regression using the same parameter list
model = KNN()
param = np.asarray(np.arange(3, 15, 1))
grid_model = GridSearchCV(model, param_grid = {'n_neighbors': param}, cv  = skf)
...
model_list = [lasso, ridge, knn, elastic]
model_names = ['lasso', 'ridge', 'KNN', 'elastic']
...
for i in range(len(model_list)):
    ...
    y_pred_train = cross_val_predict(model, x_train_std, y_train, cv = skf)
...
linear = LinearRegression()
y_pred = cross_val_predict(linear, y_train_results, y_train, cv = skf)
# calculate R_square
print "Score of the simple linear regression model ", r2_score(y_train, y_pred)
#%%
### tune lasso regression
...
## Standardize predictors
std_2 = Standardize(with_mean=False)
y_train_results_std = std.fit_transform(y_train_results)

model = Lasso_Reg()
grid_model = GridSearchCV(model, param_grid = {'alpha': param}, cv  = skf)
grid_model.fit(y_train_results_std, y_train)
# best model
lasso_2 = grid_model.best_estimator_
print "The score of the best lasso model is ", grid_model.best_score_